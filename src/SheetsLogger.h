...
void sheetLog(String url, String device, String message) {
    if (WiFi.status() != WL_CONNECTED) {
        SL_ERROR_LINE("Not connected to WiFi.");
        SL_ERROR_LINE("Message: %s", message.c_str());
        return;
    }
    ...
    if (!https->begin(*client, url)) {
        SL_ERROR_LINE("Could not connect to %s", url.c_str());
        return;
    }

    https->addHeader("Content-Type", "application/json");

    String payload = "{\"device\":\"" + device + "\",\"message\":\"" + message + "\",\"secret\":\"" + SL_SECRET + "\"}";
